...
// Perlin-ish noise + classic Perlin (cnoise) GLSL, unchanged from the
// original Beams.jsx so the visual result stays identical.
const noiseGLSL = `
...
/**
 * Full-page animated beams background, ported from the React Three Fiber
 * `Beams.jsx` component to plain Three.js so it can mount as a simple
 * client component without an R3F canvas tree.
 */
export default function BeamsBackground() {
  ...
  useEffect(() => {
    ...
    const dirLight = new THREE.DirectionalLight(0xb09bff, 1);
...
vec3 getNormal(vec3 pos) {
  ...
  gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
      ...
      material: { fog: false },
      uniforms: {
        diffuse: new THREE.Color(...hexToNormalizedRGB('#0d0b18')),
        time: { value: 0 },
        roughness: 0.35,
        metalness: 0.55,
        uSpeed: { value: 1.1 },
        envMapIntensity: 6,
        uNoiseIntensity: 1.15,
        uScale: 0.16,
      },
    });
    ...
    const beamWidth = 2.4;
    const beamHeight = 16;
    const beamGeo = createStackedPlanesBufferGeometry(beamNumber, beamWidth, beamHeight, 0, 60);

    const beamGroup = new THREE.Group();
    beamGroup.rotation.z = THREE.MathUtils.degToRad(18);
